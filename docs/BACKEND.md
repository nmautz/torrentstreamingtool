...
| `active_hash`, `active_title`, `active_file` | The current torrent (stream-now OR library playback). **`active_hash` is only meaningful paired with `library_item_id`** — teardown reads *hash set + no item* as "ad-hoc stream, delete its files", so anything that clears one must clear the other (see [GOTCHAS.md § qBittorrent](GOTCHAS.md)) |
...
| `library_item_file_count`, `library_playlist`, `library_current_file` | Multi-episode playlist state. `library_playlist` is the remaining **VLC tail** (shrinks as playback advances) |
| `library_shuffle_order` | **Shuffle Play** — the *whole* temp random file order (empty when not shuffling). Set from `play_library_item` when `req.shuffle`; cleared by any normal play/auto-play/stop/background/YouTube **and by `/api/library/unshuffle`** (leave-shuffle mid-episode). `_nav_order()` returns it when set so next/prev (`/api/vlc/next` `/prev`), the credits auto-advance (`vlc_next_file`), and `_next_file_in_item` all step the random order instead of `item.files`. Unlike `library_playlist`, it keeps the full order so Prev can walk back through history. The *persisted* shuffle preference (survives a stop) lives separately in `library.json` `progress[pid].shuffle`/`shuffle_scope` (written by `_set_shuffle_pref`, read by `find_resume_hint`) |
| `library_series_map` / `library_series_order` | **Merged-series playback** (a show whose episodes span separate single-episode items). `library_series_map` maps each playlist path → its owning item id; `library_series_order` is the full, stable cross-item path order (the analogue of `library_shuffle_order` — `_nav_order`/`_item_all_paths` return it so prev/next/credits-advance span items). Set from `play_library_item` when `req.items` is passed; empty for a normal single-item play. The progress tracker uses `_series_owner_for_path` + `_series_finalize_and_switch` to keep `library_item_id` on the currently-playing episode's item. See [GOTCHAS.md](GOTCHAS.md) § cross-item series playback |
| `library_shuffle_scope` | Pool the *live* shuffle order was drawn from (`"all"`/`"unwatched"`/`""`). Set alongside `library_shuffle_order` in `play_library_item`; surfaced in `state_snapshot` as `library_shuffle_scope` **masked to `""` whenever `library_shuffle_order` is empty** (so a stale value can't leak — other clear sites don't need to reset it). Exists so a TV→device handoff (`handoffToDevice` → `lpPlay`) can carry the scope and keep the device's persisted Resume-on-shuffle pref accurate |
...
| `tv_ui_active` | True while the TV UI dashboard kiosk should hold the host screen. Gates **every** VLC focus assertion (`vlc_focus_and_fullscreen` bails, `background_video_loop` skips) exactly like `youtube_active`; released by the `vlc("in_play")` branch / `youtube_play` when real content takes the screen, and by `tv_ui_loop` (idle hand-back / stale-input janitor). See [REMOTE.md](REMOTE.md) and [GOTCHAS.md § TV UI screen arbitration](GOTCHAS.md#tv-ui-screen-arbitration-statetv_ui_active-gates-every-vlc-focus-assertion) |
| `tv_input_last` | `time.time()` of the last HID input (any key/click/move, stamped by `_tv_input_event` from the listener hook threads). Drives the TV UI idle hand-back and the playback-janitor guard in `tv_ui_loop` |
| `host_volume_expected` | Last host OS volume (0-100) written by a legitimate path — recorded by every successful `set_system_volume`. `remote_volume_guard` reverts any endpoint volume that differs (the remote's consumer-control volume bypasses the keyboard hook) and re-routes the delta to VLC / player gain. `None` = resync on next guard tick (startup, after window-pause) |
...
- **`background_video_loop`** (every 3 s) — if `settings.background_video` is configured + enabled, `stream_status` is not `buffering`, and VLC reports anything other than `playing`/`paused`, plays the bg file via `_play_background_video()`. Sets `state.background_playing=True` and clears the playback fields (`library_item_id`, `active_hash`, titles, offers). `_play_background_video()` re-checks `_real_playback_active()` **after** its own awaits and immediately before touching VLC — the loop's decision is always stale by then, and losing that race to an episode auto-advance both stomped the screen and desynced `active_hash`/`library_item_id` into the shape that gets files deleted on Stop (see [GOTCHAS.md](GOTCHAS.md)). Naturally replaced by any user `vlc("in_play", …)` — that branch in the `vlc()` helper restores `state.vlc_volume` from `state.user_volume_before_bg` and clears `background_playing`. **Stands down while `window_mgmt_paused()`** (non-admin "Use My Computer" override) — it stops (re)starting the idle video so the user can use the desktop. See [ADMIN.md §5](ADMIN.md) and [LIBRARY_DATA.md](LIBRARY_DATA.md).
...
- **`_apply_item_schedule(item, lib)`** (not a loop) — the on-demand path the `download-schedule` / `file-schedule` / `delete-files` endpoints use after editing the model: reconciles qBit, then reactivates a **finished** item (`status` `ready`→`downloading`) when the change left a non-skip file not-on-disk (e.g. un-skipping a file on a partial download, so it actually fetches). `downloading→ready` stays owned by `library_download_monitor`.
- **`delete_item_files` (`POST /api/library/{id}/delete-files`)** — frees disk space while keeping files re-downloadable: marks the requested files `skip` (so the reconcile sets qBit priority 0, no refetch), `unlink`s their bytes, purges their `.offline_cache` HLS bundle (same `_offline_cache_key` + `shutil.rmtree` as `/recheck`), then `_apply_item_schedule` + `library_update` broadcast. Returns `{deleted, freed_bytes}`. Pairs with the skip-file disk-existence check in `get_item_files` (a deleted file then reports `complete:false` immediately instead of trusting qBit's stale `progress=1.0`).
- **`system_monitor_loop`** (every 5 s) — samples host **CPU / RAM** (psutil), **GPU** (`nvidia-smi`, best-effort, cached off on a GPU-less box), and **network** throughput + **packet-loss rate** (drops ÷ packets per window; only computed once `≥_NET_MIN_PKTS`/`≥_NET_MIN_ERRS` have flowed, and only flagged after it persists `_NET_CONFIRM` samples — so transfer ramp-up / post-boot drop spikes don't trip the banner), classifying each `ok` / `degraded` / `overloaded` (`_classify` for CPU/RAM/GPU) into `state.sys_status` (+ an `overall`). Drives the dashboard's "host busy" perf banner and the admin **System Health** card (`GET /api/admin/system-resources`); rides in every `state` SSE event as `sys_status`. See [ADMIN.md](ADMIN.md).
...
- **`remote_volume_guard`** (every 0.35 s; **OPT-IN via `REMOTE_VOLUME_GUARD=1`** and Windows-only — exits immediately otherwise) — backstop for remotes whose volume bypasses the keyboard hook. Polls `get_system_volume()`; any endpoint change not recorded in `state.host_volume_expected` (written by every successful `set_system_volume`) is reverted and its delta re-routed to VLC's amp / the YouTube `player_volume_step` yt_command, with a ~1.2 s `_remote_vol_key_ts` dedupe against hook-routed presses, a ±1 tolerance band, and adopt-actual-after-revert (both anti-fight defences for quantizing endpoints). **Off by default** — the hook alone suffices on real hardware and an always-on guard drip-stepped VLC volume; see [GOTCHAS.md](GOTCHAS.md#and-the-os-mixer-volume-guard-is-opt-in--an-always-on-guard-fights-quantizing-audio-endpoints). Resyncs (doesn't fight) during the window-pause.
- **`tv_ui_loop`** (every 5 s) — reconciles who owns the host display for the Firestick-style TV UI (`Settings.tv_ui`). While `state.tv_ui_active` (the dashboard kiosk holds the screen): drops the claim when real playback starts (janitor only — the `vlc("in_play")` branch and `youtube_play` are the primary release points — and only when the last HID input is >10 s old, protecting a just-pressed 🏠 Home), and after `tv_ui_idle_secs` with no input hands the screen back to the idle background video (`_tv_ui_hide`: `pl_forceresume` + `vlc_focus_and_fullscreen`). Keeps the UI up when no bg video is configured. Wake side: `_tv_input_event` (called from the input-listener hook threads) stamps `state.tv_input_last` and schedules `_tv_ui_show` on a key/click while idle — which pauses the bg video, launches/reuses the kiosk Chrome (`.tvui_chrome_profile`, `/?tv=1`) and forces it foreground (`_bring_tvui_to_front`, window-title marker `_TVUI_WINDOW_MARKER`). See [REMOTE.md](REMOTE.md) and [GOTCHAS.md § TV UI screen arbitration](GOTCHAS.md#tv-ui-screen-arbitration-statetv_ui_active-gates-every-vlc-focus-assertion).

Lifespan also starts the **HID remote input listener** (not an asyncio task — `pynput` threads from [remote_input.py](../remote_input.py), gated by `Settings.remote_control` / `REMOTE_CONTROL`). The hook threads call `_remote_should_handle(action)` synchronously (plain attribute reads only) and schedule `_remote_key_action(action)` onto the loop via `run_coroutine_threadsafe`; the dispatcher routes into the same VLC / YouTube control paths as the dashboard footer (volume cap, `youtube_active` routing, immediate `state` broadcast), with `home` = TV UI show + `/api/stop`. Every key/click/move is additionally fed to `_tv_input_event` (the TV UI wake/idle signal). Best-effort: `start_listener` returns None (logged once) when pynput is missing or there's no interactive desktop session; the mouse listener is best-effort on top of the keyboard hook. Stopped in lifespan teardown. See [REMOTE.md](REMOTE.md) and [GOTCHAS.md § HID remote](GOTCHAS.md#hid-remote-media-keys).
...
Cancellation: on new `/api/stream` or `/api/stop`, `state.stream_task.cancel()` is called. If `library_item_id is None`, the torrent is deleted by `_qbit_delete_transient(active_hash)` — which re-checks `library.json` and **refuses if any item is backed by that hash**, because the `state` pair it's trusting has desynced before and cost a 76-file series (see [GOTCHAS.md § qBittorrent](GOTCHAS.md)). Never call `qbit_delete` bare in a teardown path: `delete_files` defaults to `True`.
...
- **Restart-safety:** `library_download` persists the magnet + add params in `item["pending_download"]` before spawning this task; the pipeline **clears `pending_download` the moment it records the `torrent_hash`** (and on a failed add, flipping to `error`). A restart between item creation and that point would otherwise orphan a hash-less `downloading` item forever (the monitor skips items with no hash and the magnet was never persisted) — see `_recover_interrupted_downloads`.

### `_recover_interrupted_downloads` ([main.py](../main.py))
- Startup pass (awaited in `lifespan`, before `library_download_monitor` spawns). Scans the library for `status=="downloading"` items still carrying a `pending_download.magnet` and respawns `library_download_pipeline` for each, so a download interrupted mid-add (app restart before the hash was recorded) resumes instead of sticking at 0% in the ongoing list. Idempotent: re-adding a magnet qBit already has is a no-op, and the pipeline clears `pending_download` once the hash lands. Items whose hash was already recorded carry no `pending_download` and are left to the monitor.
...
### Grouped search & merged series
- **`parse_torrent_title(title)`** ([main.py](../main.py)) — classifies a raw torrent title into `{show, show_key, kind, season, episode, season_from, season_to, year}`; `kind ∈ episode|season|multiseason|movie`. Show name = text before the first structural marker → `_strip_release_tags`. **`_group_search_results(shaped, query)`** groups shaped results by `show_key` into the `groups[]` returned by `/api/search`, ordered by query relevance then seeders. `_shape_search_results` now de-dupes by info-hash (keep highest seeders). See [GOTCHAS.md](GOTCHAS.md) § search-result grouping.
- **`_tmdb_lookup_by_title(title, year, kind)`** — TMDb match by bare title (not a library item) for the search show-detail hero + episode names; process-cached by `(title,year,kind)`. Backs `GET /api/tmdb/lookup`.
- **`_build_item_files(item, profile_id)`** — the per-file payload builder shared by `GET /api/library/{id}/files` and the merged `GET /api/library/series/{key}` (so both emit identically-shaped file objects, each tagged with `item_id`). **`_merged_series_files` / `find_series_resume_hint`** flatten a show's items into one `(season,episode)`-ordered list + a series-level resume hint. **`_series_owner_for_path` / `_series_finalize_and_switch`** drive cross-item playback from the progress tracker (see AppState `library_series_map`/`_order` + [GOTCHAS.md](GOTCHAS.md)).

### `_vlc_replace_upcoming`
- Rewrites VLC's **upcoming** playlist entries *without touching the currently-playing item* — the one operation `vlc_clear_playlist()`/`pl_empty` can't do (that drops the playing item too). Reads `playlist.json`, finds the `current` leaf, `pl_delete`s every leaf after it, then `in_enqueue`s the replacement tail. Used by `/api/library/unshuffle` to swap the random tail for natural order with no rebuffer of the playing episode. Returns `False` if the in-place edit couldn't be applied so the caller can fall back to `_vlc_relaunch_playlist` + reseek (a brief rebuffer that still lands on the right order).
...
- **`qbit_first_last_piece_prio`** uses `toggleFirstLastPiecePrio` (also a toggle — read `f_l_piece_prio` first). We enable it **alongside** sequential mode on every stream-while-downloading play. Sequential alone only puts the *head* on disk, but a tail-index container (moov-at-end MP4, Cues-at-end MKV — the common non-web-optimised torrent) can't be demuxed without its last piece: VLC opens it, finds no seek table, and stops right after the buffer gate with `length` still 0, so `_vlc_wait_until_ready` and the rebuffer guard can't recover it (the "play now fails after buffering even on good torrents" bug). The last piece is a single piece regardless of file size, so grabbing it costs almost nothing and the rest still arrives sequentially from the head. `_sequential_off_when_complete` flips both this and sequential back off once the streamed file finishes. (This reverses the earlier "never enable first/last-piece prio" policy, which was the cause of the bug.)
...
- **Subtitle default policy**: VLC auto-enables a sub when a file opens, so `_apply_track_prefs` → `_apply_subtitle_policy` always sends an **explicit** `subtitle_track` on every play/prev/next (`-1` off, or a chosen ES ID on). `_apply_track_prefs` first picks the **newest** of the per-file descriptor (`subtitle_sel.at`) and the per-series pref (`updated_at`); a raw saved ES ID is only fast-pathed for an embedded pick not outranked by a newer series pick. The policy loads **every local sidecar** (`_load_all_local_subs` → `_discover_local_subs`, incl. `Subs/` folders) into VLC so all are selectable, then resolves a remembered descriptor via `_resolve_sub_descriptor` (name → sig+idx → groups → fuzzy name → lang → title → lone-option; mirrored by the client's `_lpResolveSubSel`), else picks: preferred-language track (embedded ahead of sidecars) → online auto-search (`_auto_fetch_subtitle`) → last-resort real track when an ON descriptor is pending (never silently revert to off) → off. AI sidecars fold into the same discovery (no separate step). Decision = `profile.subtitles_on` (override) ?? `settings.subtitles.on_by_default`; a remembered descriptor beats the off default. Track lists come from `_vlc_subtitle_tracks` (factored out of `/api/vlc/tracks`). See [GOTCHAS.md](GOTCHAS.md).
