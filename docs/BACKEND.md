...
Lifespan also starts the **HID remote media-key listener** (not an asyncio task — a `pynput` thread from [remote_input.py](../remote_input.py), gated by `Settings.remote_control` / `REMOTE_CONTROL`). The hook thread calls `_remote_should_handle()` synchronously (plain attribute reads only) and schedules `_remote_key_action(action)` onto the loop via `run_coroutine_threadsafe`; the dispatcher routes into the same VLC / YouTube control paths as the dashboard footer (volume cap, `youtube_active` routing, immediate `state` broadcast). Best-effort: `start_listener` returns None (logged once) when pynput is missing or there's no interactive desktop session. Stopped in lifespan teardown. See [REMOTE.md](REMOTE.md) and [GOTCHAS.md § HID remote](GOTCHAS.md#hid-remote-media-keys).
