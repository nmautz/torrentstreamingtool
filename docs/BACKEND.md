...
- **`remote_volume_guard`** (every 0.35 s; **Windows-only**, exits immediately elsewhere or when `REMOTE_CONTROL=0`) — keeps the air-mouse remote's volume off the host OS mixer. The remote's consumer-control volume bypasses the keyboard hook (see [GOTCHAS.md](GOTCHAS.md#and-the-keyboard-hook-cannot-enforce-that-alone--consumer-control-volume-bypasses-it)), so this polls `get_system_volume()`; any endpoint change not recorded in `state.host_volume_expected` (written by every successful `set_system_volume`) is reverted and its delta re-routed to VLC's amp / the YouTube `player_volume_step` yt_command, with a ~1.2 s `_remote_vol_key_ts` dedupe against hook-routed presses. Resyncs (doesn't fight) during the window-pause.
