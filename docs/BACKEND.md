...
| `library_shuffle_order` | **Shuffle Play** — the *whole* temp random file order (empty when not shuffling). Set from `play_library_item` when `req.shuffle`; cleared by any normal play/auto-play/stop/background/YouTube. `_nav_order()` returns it when set so next/prev (`/api/vlc/next` `/prev`), the credits auto-advance (`vlc_next_file`), and `_next_file_in_item` all step the random order instead of `item.files`. Unlike `library_playlist`, it keeps the full order so Prev can walk back through history |
