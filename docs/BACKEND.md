...
| `prep_paused` | True ⇒ bulk stream-prep jobs hold at the gate in `_run_offline_job`. Set by `_pause_prep` (non-admin `/api/offline-prep/pause`) and the overnight window end; cleared by `_resume_prep`. Published in `state_snapshot()` |
| `overnight_active` | In-memory flag: True while inside the admin's overnight auto-prep window. Drives the entry/exit transitions in `overnight_prep_loop` (no persisted fire-guard) |
...
- **`overnight_prep_loop`** (every 30 s) — overnight auto stream-prep. Reads `settings.overnight_prep`; on entering the `[start,end)` window (in-memory `state.overnight_active`, wrap-midnight aware) it `_resume_prep()`s and `_enqueue_library_prep()`s a bulk HLS job for every un-prepped library video file, and on leaving either `_pause_prep(kill=False)` (let the in-flight file finish, hold the rest) or runs to completion (`on_end`). See [ADMIN.md §7](ADMIN.md), [STREAMING.md](STREAMING.md), and [GOTCHAS.md](GOTCHAS.md#pausing-prep-a-paused-bulk-job-exits-its-task-and-releases-the-slot).
