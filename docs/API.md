...
| GET | `/api/library/{id}/files?profile_id=…` | Per-file list with progress + per-file `mode ∈ {now, high, idle, skip}` (effective download schedule), `dl_pct` (download %, `null` if unknown), and `complete` (file fully on disk → playable now). `complete`/`dl_pct` come from live qBit per-file progress for **any** torrent-backed item (ready included) — a skipped file is never `complete`. Response top-level adds `download_mode`, `idle_open`, `idle_configured` (see Download scheduling below) |
...
| POST | `/api/library/{id}/download-schedule` | `{mode: "now"\|"idle"}` — item-level download schedule. `idle` = Pause (download only during the idle/night window, auto-resuming there); `now` = Resume (download immediately). Sweeps per-file `now`/`high`↔`idle` overrides too, leaving explicit `skip` alone. Reconciles qBit immediately (`_apply_item_schedule`) |
