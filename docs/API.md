...
| Method | Path | Notes |
|--------|------|-------|
| POST | `/api/library/{id}/live-prepare` | `{file_path, profile_id?, start_sec?, audio_idx?}` → the on-device player's prepare entry point. **Always `ready:true`** with a `mode`: `"bundle"` (a full bundle is already cached — returns the same fields as `offline-prepare`'s ready path: `master_url, duration_sec, videos[], audios[], subtitles[], skipped_image_subs[], subs[], saved_tracks`) or `"live"` (no bundle — starts a just-in-time transcode session and returns `{session_id, playlist_url, duration_sec, audios[], subtitles[], subs[], saved_tracks}`). Live is single-quality, one audio muxed in (chosen by `audio_idx` position → saved → source default), starting at `start_sec`. **503 on macOS** / old-ffmpeg hosts. See [STREAMING.md § Live](STREAMING.md#live--instant-start-jit) |
| GET | `/api/library/live/{session_id}/playlist.m3u8` | Predicted full-length VOD media playlist for a live session — every `seg_NNNNN.m4s` listed (`#EXT-X-PLAYLIST-TYPE:VOD`, `#EXT-X-MAP:URI="init.mp4"`, `#EXT-X-ENDLIST`), computed from duration with no encoding. Touches the session's idle timer. `session_id` is 16 hex (`_LIVE_SESSION_RE`) |
| GET | `/api/library/live/{session_id}/{filename}` | Serve a live `init.mp4`, `seg_NNNNN.m4s`, or `sub_<i>.vtt`, **generating on demand**. **Blocks** (≤40 s) until the file exists rather than 404ing a not-yet-built segment (a 404 = fatal hls.js `fragLoadError`). A request for an un-reached segment (re)starts the encoder there first (`_live_ensure_encoder`); a backward/far-forward seek restarts at that segment. |
| POST | `/api/library/live/{session_id}/stop` | Tear down a live session — terminate its ffmpeg + delete its temp dir. Fired by the frontend on stop / file-advance / audio-switch (and a `sendBeacon` on `pagehide`); idle sessions are also GC'd after `LIVE_SESSION_TTL` (90 s) |
