...
| GET | `/api/library/{id}/files?profile_id=…` | Per-file list with progress + per-file `mode ∈ {now, high, idle, skip}` (effective download schedule), `prep_mode ∈ {now, idle, never}` (effective stream-prep schedule; default `idle`), `dl_pct` (download %, `null` if unknown), and `complete` (file fully on disk → playable now). `complete`/`dl_pct` come from live qBit per-file progress for **any** torrent-backed item (ready included), matched by full path with a basename fallback — so a finished partial download doesn't mislabel absent files as complete. Response top-level adds `has_torrent`, `download_mode`, `idle_open`, `idle_configured` (see Download scheduling below) |
