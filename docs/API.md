...
| GET | `/api/admin/components` | Status of installable portable deps: `{components:{ffmpeg,fpcalc,whisper,whisper_model,airplay:{label,installed,path,installable,purpose,note?,job?}}, platform, model_sizes, stt_available, nvenc}`. `nvenc` = an NVIDIA GPU is present (UI recommends a CUDA whisper build). `airplay` = the Windows iPhone screen-mirror receiver (`note` nudges you to finish its GUI installer wizard). `job` (when present) = `{status:"pending"\|"downloading"\|"done"\|"error", progress, error, message?}`. Polled while an install runs |
| POST | `/api/admin/components/install` | `{component:"ffmpeg"\|"fpcalc"\|"whisper"\|"whisper_model"\|"airplay", model?, build?}` → starts a background download+install (streamed for progress), writes the path into `.env`, clears the ffmpeg-version/NVENC/STT caches. `model` (whisper_model only) ∈ base/small/medium. `build` (whisper only) ∈ `cpu`/`cuda12`/`cuda11` (CUDA = GPU; runtime auto-falls-back to CPU via `-ng` if CUDA can't init). `airplay` downloads the uxplay-windows GUI installer + pre-enables `AIRPLAY_RECEIVER` (finish the wizard on the host, then re-`GET` to see it Installed). ffmpeg/whisper/airplay are **400** off-Windows (use the OS package manager). See [SETUP.md](SETUP.md), [AIRPLAY.md](AIRPLAY.md) |
