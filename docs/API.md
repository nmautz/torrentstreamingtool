...
| GET | `/api/admin/activity` | `{generated_at, host_busy, gates:{prep_paused, prep_pause_reason, auto_prep_mode, admin_prep_stop, vpn_secure}, activities:[{category, title, detail, status, progress, reason, resumes_after_restart, restart_note, elapsed_sec}], count, fragile_count}` — unified background-activity view for the admin **Activity** tab. Aggregates the in-memory job tables (`_offline_jobs`, `_stt_jobs`, `state.file_validation`/`file_repair`/`analysis_jobs`, `_od_sessions`) + downloading library items, each tagged with a plain-English `reason` and `resumes_after_restart` (only **Download** survives a restart — qBit persists it out-of-process; everything in-memory is fragile). `fragile_count` drives the restart warning. Polled every 4 s while the tab is open. See [ADMIN.md § Activity](ADMIN.md) |
