...
| GET | `/api/admin/logs/_bundle` | Downloads a ZIP of every file in `LOG_DIR` (deflated). Filename `streamlink-logs-YYYYMMDD-HHMMSS.zip`. 404 if no log files exist. Built into a **seekable temp file** (not streamed through a pipe) so local headers carry real CRC/sizes — a pipe-streamed ZIP uses data descriptors that Windows Explorer can't extract (see docs/GOTCHAS.md). Each member is read via its own handle so actively-written logs are still captured; temp file removed by a background task after send |
