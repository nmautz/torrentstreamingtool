...
| POST | `/api/admin/updater/apply` | `{branch?, reboot:true}` — run the full sequence: **git apply → setup.py → service uninstall/reinstall → host reboot**. `branch` accepts main/beta/alpha; passing one that differs from the current working tree triggers a switch (`git switch -C` + `git reset --hard`) in either direction. With `reboot=true` (default) the host comes down ~1.5 s after the response flushes; the admin UI handles the dead SSE connection as "reboot in progress" and waits for the host to come back. With `reboot=false`, steps 3 and 4 are skipped (code-only refresh — dev convenience). Returns `{ok, stage, message, commit?, service_reinstalled?, service_install_output?, reboot_pending?}`. Phases reported via SSE: `applying → setup → reinstalling-service → rebooting`. 409 if another operation is running |
