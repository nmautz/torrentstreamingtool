...
| POST | `/api/library/{id}/prep-all` | Pre-runs remux/transcode for every video file in an item so subsequent device-side `Save Offline` taps fetch the cached MP4 instantly. Returns `{files:[{file_path,name,status,job_id?,progress?}], total, ready, processing, errored, needs_prep, missing}` with one row per file. Coalesces with any in-flight jobs. Honours `item.offline_prefs.subtitle` for burn-in |
...
| GET | `/api/library/{id}/subtitle-options?file_path=…` | Enumerate burn-in–eligible subtitle sources for a file (defaults to first file). Combines sidecar `.srt`/`.vtt` and embedded streams (bitmap formats like PGS/DVD are skipped). Returns `{options[], current}`. Each option has `{kind:"sidecar"\|"embedded", lang, label, ...}` |
| POST | `/api/library/{id}/offline-prefs` | `{subtitle: {kind:"sidecar"\|"embedded"\|"none", ...}}` — persist per-item burn-in preference. The smart matcher in `_resolve_subtitle_source` finds the matching sub source for every other episode, so the user only picks once per series |
