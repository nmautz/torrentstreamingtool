...
| `dl-bg` / `dl-fg` | the app crossed the foreground/background line with jobs running (18.14.0). `moved` is how many in-flight transfers `migrateTasks` handed to the other session; **`disk`** (18.14.2) is confirmed on-disk bytes and is **the field to divide by**; `done` is `disk` plus in-flight bytes and therefore goes DOWN on every transition (`migrateTasks` clears them), so a rate taken from it is nonsense. `left` is `backgroundTimeRemaining` and is best-effort only — read `after` on the following `dl-bgtask-expired` for the grant that was really given. `conns` is the background session's per-host connection limit; **`cpt`** (18.15.0) says whether a continued-processing task was keeping the app alive — `moved: 0, cpt: true` on a `dl-bg` is the good case, meaning we stayed on the fast session instead of migrating. A `dl-bg` with `moved: 0` and `pending > 0` means nothing migrated and the transfers die with the assertion. **Both byte counts are only comparable between rows carrying the same `jobs` and `total`** — the denominators move as bundles start and finish |
| `cpt-register` | the `BGContinuedProcessingTask` wildcard handler was registered at launch (18.15.0). **`ok: false` means the identifier is missing from `BGTaskSchedulerPermittedIdentifiers`** and nothing else here can work |
| `cpt-submit` | a download asked the system to keep the app alive. `ok: false` with `why: "not-active"` means we declined to ask because the app was not genuinely foreground — `dasd` refuses those silently |
| `cpt-nogrant` | **submitted cleanly and never granted.** The launch handler did not run within 12 s, so we reverted to the hybrid. This is the failure Apple DTS traced to `dasd` refusing with "Foregrounded apps don't include expected identifier" while the API reported success |
| `cpt-start` | the system granted it — from here the app stays alive and the FAST session keeps running while backgrounded |
| `cpt-expired` | the system took it back. `held` is how long we had it, `moved` how many transfers were handed to the background session on the way out. Expected eventually; the fallback is exactly the old hybrid |
| `cpt-done` | every download finished and we released the hold so the system's progress UI dismisses |
