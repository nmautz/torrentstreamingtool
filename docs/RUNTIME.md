...
- **Single-instance guard.** Just before launching, `run.py` calls `dashboard_already_serving(PORT)` ([run.py](../run.py)) and, if a StreamLink dashboard is already up, **yields cleanly** (prints a notice and returns) rather than binding — Windows can't share a listening socket, so a duplicate launcher would otherwise crash-loop on `WinError 10048`. The check confirms it's *our* app via `/api/version`, so a foreign occupant of :80 is distinguished from our own instance. The daemon wrapper uses the same helper (see [DAEMON_WATCHDOG.md](DAEMON_WATCHDOG.md)). See [GOTCHAS.md](GOTCHAS.md#windows-two-instances-fighting-for-80443-single-instance-guard).
