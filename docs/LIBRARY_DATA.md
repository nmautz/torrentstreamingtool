...
**Offline sync is the exception.** `/api/sync/progress` and `/api/sync/resolve` receive **one coalesced position per file** (the device's `OfflineStore` keeps a single record), so there's no history to measure. `_offline_watch_state` applies the tail test alone and records the reported position as played (never lowering it), so an episode begun offline and finished online still completes. A scrub to the end in the *offline* player therefore still counts; closing that needs the device to accrue and send its own `played_sec`.
