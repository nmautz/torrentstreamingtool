...
>
> Job concurrency: a global `asyncio.Semaphore(OFFLINE_JOB_CONCURRENCY)` (default 1) gates `_run_offline_job` so a /prep-all on a 77-file pack queues files instead of spawning 77 simultaneous ffmpegs. Without the cap, consumer NVIDIA GPUs (Pascal/Turing) silently reject NVENC sessions past their 2–3-encoder limit and most jobs error out before producing a frame; CPU paths swamp every core. Queued jobs stay in `status="pending"` (and are surfaced by `/prep-status` and `/api/offline-active`) until they acquire the semaphore, then flip to `processing` and re-baseline `started_at` so per-job ETAs reflect ffmpeg throughput, not queue wait.
>
