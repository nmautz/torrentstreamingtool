...
`setup.py` under `STREAMLINK_AUTOUPDATE=1` **skips every `install_*` step** (winget/brew need an interactive desktop), so a box that auto-updates never downloads portable deps it didn't already have — most notably whisper.cpp **and the AirPlay screen-mirror receiver**. Rather than require a manual terminal `setup.py` run, the admin **System → Optional Components** card installs them from the web: it reuses these same helpers (`_resolve_whisper_win_url`, `_resolve_airplay_win_installer_url`, `_extract_archive`, `_find_in_tree`, the `*_candidates()` finders, the URL constants) from inside the running server (`import setup` is safe — prompts are gated under `__main__`), streams the download for live progress, and writes the path into `.env` via `_write_env_keys`. Because the files land in `tools/`, the next auto-update's `detect_tools()` + `merge_tool_paths()` re-detect them and keep `.env` current — so a one-time install persists. The **AirPlay receiver** is a GUI installer (not a portable zip), so its card download pre-enables `AIRPLAY_RECEIVER=1` and prompts you to finish the wizard on the host, then Refresh (see [AIRPLAY.md](AIRPLAY.md)). See [ADMIN.md](ADMIN.md) and `main.py` `_run_component_install` / `/api/admin/components`.
