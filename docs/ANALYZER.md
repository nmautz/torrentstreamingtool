...
- **Subprocess work** (ffmpeg / fpcalc / ffprobe via `_media_duration`, `_fpcalc_raw`) goes through `asyncio.to_thread` — a thread is fine because the subprocess releases the GIL while it runs. Fingerprinting runs `FP_CONCURRENCY` (2) episodes at a time behind a semaphore
