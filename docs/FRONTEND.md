...
A show whose episodes were downloaded individually is many separate items sharing one `series`. In the library `renderLibrary` collapses such a group (`gItems.length>1 && gItems[0].series`) into **one show tile** (`_libShowTileHtml`) — Episodes → `openSeriesPage(seriesKey)`, ▶ Play/Resume → `playSeries(seriesKey, mode)`. `openSeriesPage` reuses the *same* `#episodePage` renderers but sources `epFiles` (each carrying `item_id`) from `GET /api/library/series/{key}`, sets `epSeriesKey` + `epFileItem` (path→item_id), and enriches metadata via `/api/tmdb/lookup` (member items only cache their own seasons). Play actions (`epPlay`/`epPlayFrom`/`startShuffle`) compute a parallel `items[]` via `_epItemsForPaths` and pass it through `playLibraryWithChooser` → `playLibraryFiles`/`lpPlay` → the `/play` body's `items[]` (TV) or `lp.playlistItems` (device) so **both** players span item boundaries. Per-file/per-bulk management stays item-aware: `_epCardHtml` keys off `f.item_id`, `epToggleWatched` uses `_epItemFor(path)`, and `epMarkWatched`/`epDeleteFiles` fan out per item via `_epGroupByItem`. Series-mode hides the per-item chrome that can't span items (rename/metafix/ondemand, the ZIP "Download selected"). **Device:** `lp.playlistItems` + `_lpLoadIndex` re-point `lp.itemId` to the current file's owner on every advance so prep/stream/progress target the right item (see [GOTCHAS.md](GOTCHAS.md) § cross-item series playback); only the app's *offline bundle* prefetch-ahead stays per-item.

#### Grouped search + show detail (`#searchShowPage`)

`doSearch` renders `groups[]` from `/api/search` as **one card per show** (`_ssGroupsById`); a movie-only single-torrent group taps straight to `openDownloadModal`, otherwise `openSearchShow(group)`. `#searchShowPage` is a `#episodePage`-styled full-screen view with a TMDb hero (via `/api/tmdb/lookup`) and two sections — **Single Episodes** (`_ssBuildEpisodes` buckets `kind==="episode"` results by `(season,episode)`; each row → `ssOpenSourceSheet` picks a torrent → `_ssDownloadOne`; **Bulk download** `ssOpenBulkSheet` → Auto highest-seeders or Choose-per-episode → `_ssRunBulk` fires throttled downloads) and **Packs** (`kind ∈ season|multiseason|movie` → `ssDownloadPack` → the Add-to-Library modal). Every episode/pack download is tagged `series = group.title` so it coheres into one library show (via `openDownloadModal(magnet,title,{series})`).
