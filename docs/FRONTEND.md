...
- **Mobile landscape.** A `@media (orientation:landscape) and (max-height:500px)` block in the `<style>` head keeps short landscape phones usable (tablets ≥744px tall are excluded). The episode page flips to a **two-pane split** (`#episodePage{flex-direction:row}`): the hero (`#epHero`) is a scrollable ~¼-width left column with the series art + description (`#epHeroContent` stacks vertically), and the season tabs + episode list + actions — wrapped in `#epRightPane` — fill the right ¾. (`#epRightPane` is a `flex-col flex-1` wrapper that's transparent in portrait, so the same DOM serves both orientations.) The block also strips the playback UI to **transport-only** so nothing scrolls. The **footer collapses to just the show name + the fullscreen button**: the seek bar (`#seekBarWrapper`), the Audio/Subs row (`#trackControls`), the PLAYING badge (`#statusBadge`), and every controls-row button except `#fullscreenBtn` (`#playerControlsRow > button { display:none }`, then `#fullscreenBtn` re-shown) are hidden — the overlay (one tap away) carries the full controls. The status row (name/time) is left intact. In the fullscreen-controls tile grid (`#fcTileGrid`) the seek bar + transport rows (play/seek, ep-nav, stop/device) keep their default `flex-1` so they grow to fill the grid (no dead space at the bottom; `overflow-y:auto` is a safety net only). The Volume row (`#fcVolRow`) and Audio/Subs row (`#fcTrackRow`) aren't hidden but **relocated into the "More" sheet** (`#fcMoreBody`, above Night Mode) by `_fcSyncMoreRows()` — it moves the **real DOM nodes** (not copies, so the single `loadTracks`/volume-handler sync path is untouched) and moves them back to their original grid slots (before `#fcStopRow`) in portrait/large layouts. The function runs on `openFullscreenControls()` and on a `matchMedia("(orientation:landscape) and (max-height:500px)")` change listener so live rotation is handled. If the More sheet outgrows one screen as controls are added, it can be paginated later.
