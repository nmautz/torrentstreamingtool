...
- **Filters** — an **All / Shows / Movies** kind row (`exSetKind`) plus a scrollable **genre chip** row (`#exGenreChips`, from the merged `/api/tmdb/genres` list, filtered to chips valid for the active kind; `exToggleGenre` toggles). Switching kind re-renders chips and drops an active genre the new kind doesn't have. Selecting a genre switches to the grid view.
- **Grid view (`#exploreGridWrap`)** — a paged poster grid (`_exOpenGrid` with `{kind, section, genre, title, genreMode}`; `exLoadMore` fetches page N+1 under `withInflight("ex_grid")`, `#exMoreBtn` hides at `total_pages`). Genre grids (`genreMode`) also get **Popular / Top Rated / New** sub-chips (`#exGridSections`, re-opening the grid with that `section`). **‹ Back** (`exCloseGrid`) clears the genre and returns to rails. `_exSeq` invalidates in-flight fetches on any view/filter change.
- **Tiles** (`_exTile(c, grid)`) — 2:3 poster (w185, native `loading="lazy"`), title, TV/Movie chip, year, ★ rating; fixed-width in rails, fluid in the grid. A click calls **`openSearchShowFromTmdb(c)`** — the candidate shape matches `/api/tmdb/search`, so the whole show page (trailer, episode skeleton, torrent search, downloads) is shared with Search.
