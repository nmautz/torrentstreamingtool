...
The **Files** expander appears on a downloading item and **also on a finished partial item** (`download_partial && torrent_hash`), so the full per-file controls stay available after the kept files complete — clicking **Now**/**Top** on a ⊘-skipped row re-downloads it (the server flips the item back to `downloading`). `renderLibrary` re-populates any open expander after a re-render (a status flip rebuilds the card). The **episode picker** marks a not-on-disk episode ⊘ Not downloaded (skipped → a **⬇ Download** button via `epDownloadSkipped`) or ⬇ Downloading…% (actively fetching) and blocks play either way.
