...
- **Names** — `play pause resume hourglass check x warn bolt star menu gear up down moon ban lock unlock key phone folder dots chev-up chev-down chev-right ext`. Semantic mapping: `resume` = the old `↩` (continue-from-position), `ban` covers both `🚫` and `⊘` (not-downloaded / skipped / never), `x` covers close ✕ **and** failed ✗, `dots` is the Metro square-dot "More" ellipsis, `ext` = external link ↗.
