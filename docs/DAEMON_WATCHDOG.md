...
5. On Windows: calls `run.setup_windows_firewall(80)` and `(443)` if certs exist (idempotent), then `run.apply_windows_power_settings(...)` with the `.env` `WINDOWS_ADMIN_USER`/`WINDOWS_ADMIN_PASSWORD` credentials — re-asserts the power/sleep-buttons-to-"Do nothing" tweak in case the elevated install never ran (registry-verified skip when already applied; see [RUNTIME.md § Windows power / sleep buttons](RUNTIME.md))
...
- While elevated, also runs `run.apply_windows_power_settings()` — sets the physical power/sleep buttons to "Do nothing" via `powercfg` (needs the admin token this path already holds). Doing it here means most installs never need the `WINDOWS_ADMIN_USER`/`WINDOWS_ADMIN_PASSWORD` runtime fallback. See [RUNTIME.md § Windows power / sleep buttons](RUNTIME.md)
