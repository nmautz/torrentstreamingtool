...
### …and the keyboard hook CANNOT enforce that alone — consumer-control volume bypasses it

Real-world failure (v10.1.0): volume suppression + VLC routing worked in principle, yet the remote still changed the Windows system volume and VLC's never moved. Cause: air-mouse remotes typically emit volume as HID **consumer-control usages** (Usage Page 0x0C, Volume Increment/Decrement), which Windows' audio stack applies to the endpoint volume **directly** — a `WH_KEYBOARD_LL` hook either never receives a `VK_VOLUME_*` event for the press at all, or receives a synthesized one whose suppression does not undo the already-applied mixer change. Fix: **`remote_volume_guard`** (Windows-only lifespan task, ~3 Hz `get_system_volume` poll through the winvol helper child). Contract: every legitimate OS-volume writer must go through `set_system_volume` — it records `state.host_volume_expected` on success, and the guard treats ANY other endpoint change as a remote press: revert to expected, re-route the delta to VLC / `player_volume_step`. If you add a new OS-volume write path that bypasses `set_system_volume`, the guard will fight it. The `_remote_vol_key_ts` dedupe window (~1.2 s) keeps remotes that emit **both** a consumer usage and a VK event from double-stepping VLC (hook already routed the press → guard reverts only). The guard resyncs instead of reverting during `window_mgmt_paused()` — desktop users own their mixer. Windows volume OSD still flashes on each press (the OS applies the change before we revert; only HID-level filtering could stop that — not worth it).
