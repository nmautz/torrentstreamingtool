...
- **Do NOT add `first_pts=0` to the aresample.** Anchoring audio to PTS 0 against a copied video whose first PTS isn't 0 *introduces* a constant offset — the opposite of the goal. The filter must correct drift/gaps without forcing the start.
- **The repair tool's detector has a blind spot.** **Detect & Repair Audio Sync** (`_bundle_audio_sync_delta`, `POST /api/admin/hls-resync`) flags a bundle by comparing the **total** `#EXTINF` duration of its audio rendition vs its video rendition. That catches the **drift / gap** class (where the audio ends up a different length) but a *pure constant offset* leaves both totals equal and is **undetectable** this way. If a constant offset is ever reported on a specific source that the resync tool won't flag, the next lever is `-copyts -start_at_zero` (shifts all streams to a common zero base, preserving relative sync) or transcoding the original rung instead of copying it — neither is applied by default because they cost more and the async-resample recipe covers the common cases.
