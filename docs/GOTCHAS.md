...
**This is TV/VLC only.** The on-device HLS player is single-item by construction: its `stream-ondemand` / `offline-prepare` endpoints validate `file ∈ item`, and progress sync posts a single `lp.itemId`. So a merged series played *to a device* plays the tapped episode from its own item and does **not** auto-advance across item boundaries (`pcChoose("local")` trims the playlist to the leading run owned by the first item). Extending cross-item to the device means threading a per-file item map through `lp` + those endpoints.
...
`parse_torrent_title` ([main.py](../main.py)) extracts a result's show name from the substring **before** the first structural marker (`S01E03`, `S01`, `S01-S05`, `Season 2`, `Complete Series`, …), then strips release tags. This is deliberate: a torrent titled `Better Call Saul S06E11 Breaking Bad …` or `Moonshiners S15E12 Braking Badly …` matches a "breaking bad" search but is a *different show* — cutting at the first marker yields "Better Call Saul" / "Moonshiners" so they group into their own cards instead of polluting the searched show. Kind classification order matters: multi-season ranges (`S01-S02-S03…`, prefer the chained list over the 2-season range regex so the full span is captured) → episode → single season → movie. Change the regexes and you change grouping; validate against real indexer titles, not just synthetic ones.
