...
**Every analysis trigger must go through `_schedule_analysis` (synchronous in-flight guard).** The old guards checked `job.status == "running"`, but that status is only set *inside* `_run_series_analysis` after it `await`s the library — so between `create_task(...)` and the status flip, a burst of triggers (a `/prep-all` firing the post-prep hook once per file, or the admin **Re-fingerprint** racing the 30 s `background_maintenance_loop`) each saw "not running", queued their own task, and the tasks serialized on the per-series lock → the whole pass replayed several times (the progress bar restarting 0→100 over and over). Fix: `_schedule_analysis(series_key, trigger)` adds the key to the module-level `_analysis_inflight` set **synchronously, before any await**, and is the *only* place that should `create_task(_run_series_analysis(...))`. `_any_analysis_running` and `_find_unfingerprinted_series` also treat in-flight as busy. Don't reintroduce a bare `asyncio.create_task(_run_series_analysis(...))` at a trigger site, and don't gate solely on `job.status`. Each run logs `run START`/`run END` with its `trigger` to `streamlink_app.log` — check there first when a re-run looks runaway.
