...
- **NVENC ⇒ NVDEC, but transparently (`-hwaccel cuda`, *not* `-hwaccel_output_format cuda`).** On the NVENC path `-hwaccel cuda` is inserted before `-i` so decode runs on the GPU (without it the CPU does full-res decode + every software scale and pegs at 80-90% while the GPU idles ~50% — the symptom that prompted this). Keep it the **transparent** form: omitting `-hwaccel_output_format cuda` makes ffmpeg download frames to system memory for the existing CPU `scale` filter **and** silently fall back to software decode for any codec NVDEC can't handle. Forcing frames to stay in VRAM (to feed `scale_cuda`) removes that fallback and **hard-fails** on exotic source codecs — don't do it; Windows is the primary target and a regressed prep is worse than a slightly warmer CPU. Only added when a rung actually decodes (`needs_decode`) — a lone stream-copy rung has nothing to accelerate.
