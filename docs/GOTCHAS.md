...
Fourth: **a standard-user process cannot silently gain admin rights — there is no clever bypass.** This bit two attempts: (a) `schtasks /Create … /RL HIGHEST` → *"Access is denied"* because **creating** an elevated task needs an already-elevated caller; (b) `CreateProcessWithLogonW` with the admin creds → the credential logon returns a UAC-**filtered** token, so the per-machine `msiexec /qn` fails **1603**. The StreamLink server runs **non-elevated** on purpose (the TV box's interactive user is a *standard* account, and the server must run in that user's desktop session to control VLC — running as a different admin user would lose the desktop). The working design is the **install-time elevation helper**: `daemon.py` `_windows_register_elevation_helper` creates a `/RL HIGHEST` task (`StreamLinkElevate`) during the elevated `--install` (using `WINDOWS_ADMIN_USER`/`PASSWORD`, which must be a **local admin**); at runtime `_run_elevated_windows` **runs** that existing task (allowed without elevation) via a rewritten `.elevate\run.cmd`, reading the exit code from `.elevate\result.txt`. So enabling silent installs **requires re-running `python run.py --install` once** after setting the creds. The password is **plaintext in `.env`** *and* stored by Task Scheduler; `.elevate\run.cmd` is server-writable → elevated-exec trust equals the repo dir. If you don't want stored creds, install the receiver `.msi` manually once (it's a one-time thing).
