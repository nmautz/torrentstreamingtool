...
- **The offset detector must diff against the SOURCE, not assume zero.** **Detect & Repair Audio Sync** (`POST /api/admin/hls-resync`) flags on *either*: (1) `_bundle_audio_sync_delta` — audio-vs-video total `#EXTINF` duration divergence (the drift class); (2) `_bundle_introduced_av_offset` — the bundle's audio-vs-video first-pts gap **minus the source's intended offset** (`_source_av_offset`). The duration check alone is blind to a constant offset (both renditions are the same length) — which is exactly why the first cut of the tool reported "no issue" on a visibly-desynced show. But measuring the bundle's raw offset isn't enough either: a source with *genuinely* delayed audio (audio really starts later) produces a non-zero bundle offset that's **correct** — only the part the prep *added* (bundle offset − source offset) is a bug. Diff against the source; keep both detectors.
