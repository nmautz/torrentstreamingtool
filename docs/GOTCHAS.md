...
### OS-volume (pycaw/COM) must run on ONE long-lived thread — never CoUninitialize per call

The host OS mixer is driven via pycaw/COM on Windows (`_windows_volume_op`, used by `set_system_volume`/`get_system_volume` → the YouTube-on-TV volume path and the global Host Volume slider). **Do not** `CoInitialize → work → CoUninitialize` on every call. Tearing the COM apartment down between calls leaves comtypes' cached interface metadata/proxies dangling; a later access — often the GC of a COM pointer on a *different* `asyncio.to_thread` pool worker — is a **native access violation that kills the whole Python process before Python can raise or log anything**. The symptom is maddening: the first few volume changes work, then the server just vanishes with nothing in the logs.

The fix (in place since 3.12.1): all Windows volume ops run on a single dedicated worker (`_win_volume_executor`, `max_workers=1`). `CoInitialize` is called exactly once on that thread (guarded by a `threading.local` flag) and **never** uninitialized — the apartment stays alive for the life of the process, no COM pointer is created on one thread and released on another. `set_system_volume`/`get_system_volume` route through this executor on Windows only; POSIX keeps `asyncio.to_thread` because `osascript`/`pactl`/`amixer` are plain subprocesses with no COM. If you add a new OS-volume caller, go through `set_system_volume`/`get_system_volume` — don't call `_windows_volume_op` or pycaw directly from an arbitrary thread.
