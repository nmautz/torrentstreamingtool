...
Fourth: **self-elevating installs** (`_run_elevated_windows`) use **`CreateProcessWithLogonW`** (Secondary Logon), **not** `schtasks`. The schtasks approach (`/Create … /RL HIGHEST`) fails with *"ERROR: Access is denied."* on a non-admin host because **creating** a HIGHEST/run-as-other-user task requires the *caller* to already be elevated — and the StreamLink service runs as a **standard user** (daemon.py deliberately omits `/RL HIGHEST`). `CreateProcessWithLogonW` works from a standard user: it launches the command as a *different* admin account using supplied credentials, and because that's a fresh full admin logon (not the standard user's token-filtered session) the child gets the elevated token with no consent dialog. Requirements: `WINDOWS_ADMIN_USER`/`WINDOWS_ADMIN_PASSWORD` must name a **local Administrator**, the **Secondary Logon service** (`seclogon`) must be enabled, and the password can't be blank. The password is **plaintext in `.env`**. The installed StreamLink service running **non-elevated** is *why* this stored-cred path exists at all — the server can't elevate on its own.
