...
- **Kind order**: sequel-season episode dash → multi-season → episode (`SxxExx`/`NxNN`) → single season (incl. ordinal `2nd Season`) → **absolute-numbered anime** (`EP1168`, `Show - 1168`, `Show 1168 [`; season 1, episode = the number; skips years/resolutions; the bare-trailing-number form needs 3–4 digits so `Ocean's 11` isn't an episode) → movie.
- **Grouping key** lower-cases the show, drops **all punctuation** (so `Frieren: Beyond Journey's End` and `Frieren Beyond Journeys End` merge — scene styling varies per group), then strips a trailing file extension and a **trailing bare year** so `Inception 2010` and `Inception (2010)` collapse to one card. (Trade-off: this also merges e.g. `Blade Runner` and `Blade Runner 2049` — acceptable; they're both "blade runner" sources.) Country tags are *not* stripped, so `The Office US` stays distinct from a bare `The Office`. A cut mid-parens (`Show (Season 1) …`) has its dangling `(` trimmed so it doesn't mint a phantom `Show (` group.
...
- **`MULTi` means multi-*audio*, `Multi-Subs` doesn't** — the `multi` match carries a `(?![ ._-]?subs?)` lookahead.
- **Untagged (`""`) counts as original-audio for the "Original + subs" preference but *not* for "English"** — fansub releases carry no marker at all, and plain English content (Breaking Bad) is also untagged, which is exactly why the preference chips only render when the loaded results actually contain `dual`/`dub`/`other` (`_ssHasAudioVariety`).
