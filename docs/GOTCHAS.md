...
For subtitle *timing* accuracy `_run_whisper` passes `-dtw <preset>` (Dynamic Time Warping token alignment — fixes lines lingering across long pauses) plus `-ml`/`-sow` (word-boundary cue splitting). The footgun is `-dtw`: its value names the model's **architecture** (e.g. `base`, `large.v3`), and whisper.cpp **fails the whole run** if the preset's alignment heads don't match the loaded model. So never pass `-dtw` blindly off the model filename — `stt._dtw_preset()` maps `model_name()` through `_DTW_PRESETS` and returns `""` (DTW disabled) for anything it can't map confidently. If you add a new model size/variant to the Components picker, add its preset to `_DTW_PRESETS` too, or it'll silently lose DTW precision (still works, just coarser timing). `-ml`/`-sow` are always safe to pass. DTW needs no extra download (heads are built into whisper.cpp). For the *cross-window* drift that accumulates over long media, `_run_whisper` passes `--vad` (when the optional Silero model is installed — see the next gotcha). See [STT.md](STT.md) § Timing precision.

### AI subtitles (STT): do NOT force whisper `--max-context 0`

v4.6.0 added `-mc 0` (`--max-context 0`) unconditionally to curb the repetition/hallucination loops that corrupt cross-window timestamps. It does curb them — but it also strips the prior-window context the model relies on to decode ambiguous audio, which **drastically degraded transcription quality** (garbled/wrong words), *worst* when combined with VAD because VAD's short speech segments then each decode with zero shared context. Reverted in v4.6.1 (whisper uses its default context again); the pipeline generation was bumped to `g3` so the bad subs flag stale for regeneration. **Use VAD, not `-mc`, for drift mitigation** — VAD re-anchors *timing* per speech region without touching decoding context. Don't re-add `-mc 0` thinking it's a free anti-drift win; it isn't. See [STT.md](STT.md) § Timing precision.

### AI subtitles (STT): the Silero VAD model must NOT be discovered as a transcription model

The optional VAD model (`ggml-silero-v5.1.2.bin`, ~1 MB) is also a `ggml-*.bin`, so a naive glob would offer it as a tiny "transcription model" — `stt.model_name()` would read `silero-v5-1-2`, DTW would disable, and transcription would be garbage. Guards: it lives in `tools/whisper/vad/` (a **separate dir** from `models/`), `setup.whisper_model_candidates()` explicitly **excludes `ggml-silero-*`**, and it's discovered by its own `whisper_vad_candidates()` → `.env` `_WHISPER_VAD_MODEL`. Two more footguns: (1) `--vad` only exists on whisper.cpp ≳ v1.7.5, so a user-built Linux/macOS binary may lack it — `stt._whisper_supports_vad()` probes `--help` (cached per binary) and we only pass `--vad` when confirmed, with a runtime retry that drops `--vad` as a backstop; (2) VAD is **optional** — never add it to `stt.is_available()`, and the `whisper_vad` component is installable on **every** OS (it's portable data, not the Windows-only whisper *binary*). See [STT.md](STT.md) § Timing precision.

### AI subtitles (STT): auto-update must not reset the whisper model to `base`

`setup.detect_tools()` used to pick the whisper model with `next(iter(whisper_model_candidates()))` — the first `ggml-*.bin` rglob yields. When an admin had swapped to `small`/`medium` via the Components card (which leaves the original `ggml-base.bin` on disk), a non-interactive `setup.py` re-run (the auto-updater) would re-detect `base` and `merge_tool_paths()` would clobber `_WHISPER_MODEL` back to it. Fix: `detect_tools()` **prefers the model already recorded in `.env`** when the file still exists, only auto-picking a candidate when it's gone. If you add another "user-selectable, multiple-on-disk" data file detected this way, apply the same prefer-existing-`.env` rule.
