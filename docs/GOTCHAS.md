...
**The one sanctioned exception** is the on-device player: a locked document means mobile browsers can never auto-hide their URL bar (that only happens on real *root* scrolls — an inner scroll container never collapses it, which is also why the rule targets `html:has(...)`, not `body`), and iPhone Safari has no element-fullscreen API, so swipe-to-hide is the only path to true fullscreen there. While `#localPlayer.lp-active:not(.lp-tiny)` is open (coarse pointers only), the root regains `overflow-y:auto` over a hidden 45vh `body::after` buffer. Don't "clean up" that rule, and don't widen it to other overlays without the same justification — every root-scrollable state reintroduces a slice of the loose-page behavior. See [FRONTEND.md](FRONTEND.md) § Layout.
