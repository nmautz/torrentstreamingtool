...
- **Edit** button → opens the inline editor. At the top:
  - **Mode** toggle (**Automatic** | **Manual / Templates**) → `POST /api/admin/library/{id}/skip-mode`. Automatic is the fingerprint-clustering pipeline; Manual extrapolates admin-marked intro **templates** (credits stay auto). **Switching mode does *not* re-fingerprint** — see the explicit button below.
  - **Intro Templates** list (manual mode) — each template's name · source file · `start–end`, with a **Delete** button (`DELETE …/skip-template/{id}`). Adding/deleting a template also does **not** auto-run. A hint shows when templates exist but the series is still on Automatic.
  - **⟳ Re-fingerprint Now** — the dedicated trigger (`POST …/analyze`) that runs analysis for the whole series in its current mode (Manual ⇒ apply templates; Automatic ⇒ re-cluster). This is intentional: the admin marks all templates / flips the mode first, *then* runs it once, rather than the analyzer churning on every edit.
  - Per file: three numeric fields (intro start, intro end, credits start) + a **▶ Mark Intro On Device** button that opens the episode on the dashboard's on-device player in capture mode (a new tab) to mark a template visually (admin-only — see [STREAMING.md](STREAMING.md)). Save calls `PATCH /api/admin/library/{id}/skip-data`; manual edits set `analysis.source="manual"` and are **never overwritten** by analyzer/template re-runs (a `source="template"` auto-applied intro is overwritten normally).
