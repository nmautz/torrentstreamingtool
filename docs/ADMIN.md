...
Lists five components — **ffmpeg**, **fpcalc**, **whisper.cpp** (binary), **whisper model**, and **AirPlay receiver** (iPhone screen mirror, Windows) — each with an Installed/Missing badge, its resolved path, and an Install/Reinstall button. The whisper model has a size picker (base/small/medium, all multilingual). whisper.cpp has a **build picker** — CPU, GPU · CUDA 12 (~440 MB), or GPU · CUDA 11 (~60 MB); when the `nvenc` probe reports an NVIDIA GPU the card recommends a CUDA build (much faster STT) and a CUDA build is preselected. A CUDA build auto-offloads to the GPU and falls back to CPU at runtime if the driver can't initialize CUDA (so a wrong pick degrades rather than fails). Installs stream on the host with a live progress bar.

The **AirPlay receiver** is the one component backed by a Windows **`.msi`** (it registers Apple Bonjour), not a portable zip — its button reads **Get installer**: the server downloads the uxplay-windows installer into `tools/airplay/` and pre-enables `AIRPLAY_RECEIVER=1`. If you've stored **Windows admin credentials** (`WINDOWS_ADMIN_USER`/`WINDOWS_ADMIN_PASSWORD` in the env-keys editor under the Updates tab), it installs **silently with no UAC prompt** (`msiexec /qn` run elevated via a one-shot credential-backed Scheduled Task). Otherwise it best-effort launches the wizard and an amber note prompts you to approve UAC + finish it on the host's TV desktop, then hit **Refresh** (which flips the badge to Installed once the binary is detected). See [AIRPLAY.md](AIRPLAY.md) for the elevation mechanism + security caveats.

Mechanics ([main.py](../main.py) `_run_component_install`): reuses `setup.py`'s URL/extract/detect helpers (safe to `import setup` — its prompts are gated under `__main__`), streams the download via httpx for progress, extracts into `tools/`, writes the path into `.env` (`_write_env_keys`), and clears the ffmpeg-version / NVENC / STT-availability caches so the new binary takes effect without a restart. Because the files live in `tools/`, the next auto-update's `detect_tools()` + `merge_tool_paths()` re-detect them — a one-time install persists. **ffmpeg, whisper.cpp, and the AirPlay receiver are Windows-only here** (off-Windows the button is disabled with an "OS package manager" note); fpcalc and the model install on any OS.

- `GET /api/admin/components` → per-component status + any in-flight install job.
- `POST /api/admin/components/install` → `{component, model?}`; 400 for ffmpeg/whisper/airplay off-Windows.
