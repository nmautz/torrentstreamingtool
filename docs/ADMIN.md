...
STT (whisper.cpp) config — enable toggle, preferred default language, English-translation toggle, and an unavailable banner when whisper isn't installed. A status line reports whether **VAD timing-drift reduction** is active (the `vad` flag = model installed AND the build supports `--vad`) and nudges installing the VAD model from Optional Components otherwise. See [STT.md](STT.md). `GET`/`POST /api/admin/stt`.
