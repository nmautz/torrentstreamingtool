...
The **AirPlay receiver** is the one component that's a **GUI installer**, not a portable zip — its button reads **Get installer**: the server downloads the uxplay-windows installer into `tools/airplay/`, pre-enables `AIRPLAY_RECEIVER=1`, and best-effort launches the wizard; an amber note then prompts you to finish the wizard on the host's TV desktop and hit **Refresh**, which flips the badge to Installed once the receiver binary is detected. See [AIRPLAY.md](AIRPLAY.md).
