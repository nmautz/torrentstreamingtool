...
- Other per-item endpoints (`/files`, `/play`, `/download`, etc.) still do not check `admin_only`; the gate is at list-time + on these global indicators. A user who somehow already holds a hidden item's id can still hit those — the lock is a UI-visibility gate, not a hard access boundary (see [LIBRARY_DATA.md](LIBRARY_DATA.md) on soft PINs).
