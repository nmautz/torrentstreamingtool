...
| Disable the power/sleep buttons (Windows) | Automatic | applied during `run.py --install` (elevated) or at launch; needs `WINDOWS_ADMIN_USER`/`_PASSWORD` in `.env` only if neither ran elevated — see [Configuration reference](#configuration-reference) |
...
You can drive playback from the couch with a cheap **air-mouse wireless remote** — plug its 2.4 GHz USB dongle into the StreamLink host (the machine connected to the TV) and it works out of the box: no pairing, no configuration. These remotes show up to Windows as an ordinary keyboard+mouse, and StreamLink listens for their media keys globally.

**Supported remotes** — anything that sends standard media keys, which is essentially every air-mouse / HTPC remote sold for TV boxes:
...
**What each button does** (active whenever something is playing — VLC or YouTube-on-TV; when nothing is playing the buttons are left alone):
...
No setup needed: the kiosk launches itself on the first button press (Chrome or Edge required, same as YouTube-on-TV). On the first wake, pick a profile with the pointer — it's remembered.

Notes:

- **Windows**: the handled keys are consumed while claimed, so the Windows volume overlay won't pop, a focused VLC won't double-react, and Home won't open Edge. Everything else passes through.
- **Linux/macOS**: the keys are observed but can't be selectively consumed — your desktop environment may also react to the volume keys, and the Home button isn't available. On macOS you must grant Python the **Input Monitoring** permission (without it the TV UI never wakes).
- Set `REMOTE_CONTROL=0` in `.env` to turn the listener off, `TV_UI=0` to disable just the kiosk, `TV_UI_IDLE_SECS` to change the hand-back delay. Details in [docs/REMOTE.md](docs/REMOTE.md).
...
| `QBIT_DOWNLOAD_PATH` | _(under your user folder)_ | Where files are saved (created automatically). If you also set `LIBRARY_PATH_2..4` (or add folders in the dashboard), a download you don't give a folder goes to whichever of them has the **most free space** — so one drive can't fill up while another sits empty |
| `LIBRARY_PATH_2` / `_3` / `_4` | _(empty)_ | Optional extra storage folders, typically on other drives. They show as one-tap destination chips in the download modal and share the free-space auto-pick above. Any path can be opted out of that auto-pick — see below |
...
| `REMOTE_CONTROL` | `1` | Air-mouse remote media keys — set `0` to disable (see [Wireless remote control](#wireless-remote-control-optional)) |
| `TV_UI` | `1` | Firestick-style dashboard kiosk on the TV — set `0` to disable |
| `TV_UI_IDLE_SECS` | `120` | Seconds of no remote input before the TV UI hands back to the background video |
| `REMOTE_VOLUME_GUARD` | `0` | Advanced: only set `1` if the remote's volume buttons change the **Windows system volume** even while StreamLink is running — enables an OS-mixer watchdog that reverts those changes and reroutes them to VLC |
| `WINDOWS_ADMIN_USER` / `WINDOWS_ADMIN_PASSWORD` | _(empty)_ | Windows only, optional. Admin credentials for elevated host tweaks — StreamLink sets the physical **power/sleep buttons to "Do nothing"** so a stray press can't suspend the host mid-stream. `run.py --install` applies this while elevated, so most installs never need these; set them (here or in the admin panel → Updates tab) only if launches keep warning that the buttons couldn't be disabled |

> **Where downloads go.** A download you don't give a folder lands on whichever configured folder has the **most free space** — so one drive can't fill to 100% while another sits empty. The download modal shows the choice (`Auto — most free space`) and you can always override it with a chip or by typing a path. If a folder is a bad place to *put* downloads even though it's a fine place to *keep* media — a NAS, an archive drive, a disc you only read from — open **Storage** in the dashboard and click its **Auto** button to opt it out. It stays a library folder and a one-tap destination; it just never wins the automatic pick. (Opting a folder out needs a profile PIN or the admin password, like removing one does.)
...
```
Search → TMDb (real shows/movies, poster list)
  ↓
Pick a show → TMDb season/episode skeleton (no torrents yet)
  ↓
"Search packs" / "Search episodes" → Jackett API → results sorted by seeders
  ↓
