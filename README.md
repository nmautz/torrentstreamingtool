...
The remote's other buttons (arrows, OK, Back, Home, mouse pointer) are **not** intercepted — they keep working as normal keyboard/mouse input. A TV-friendly on-screen UI driven by those keys is planned.

Notes:

- **Windows**: the handled keys are consumed while playback is active, so the Windows volume overlay won't pop and a focused VLC won't double-react. Everything else passes through.
- **Linux/macOS**: the keys are observed but can't be selectively consumed — your desktop environment may also react to the volume keys. On macOS you must grant Python the **Input Monitoring** permission.
- Set `REMOTE_CONTROL=0` in `.env` to turn the listener off. Details in [docs/REMOTE.md](docs/REMOTE.md).
