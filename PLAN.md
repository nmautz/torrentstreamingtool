...
- [x] **37.12** (v5.4.1) **Fixed the elevation mechanism.** The schtasks approach failed with "Access is denied" because creating a `/RL HIGHEST` task needs an *elevated caller* and StreamLink runs as a standard user. Switched `_run_elevated_windows` to **`CreateProcessWithLogonW`** (Secondary Logon — works from a standard user, launches as the admin account, child gets the full elevated token). `_split_win_account` parses `user` / `HOST\\user` / `user@domain`. Clear WinError messages (1326/1058/…). Requires a local-admin account + the Secondary Logon service. Docs (AIRPLAY/GOTCHAS), CHANGELOG, version → 5.4.1.
