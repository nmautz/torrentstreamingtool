...
- [x] **5.2** Smart Skip: audio fingerprinting to detect and skip intro/credit sequences on library files
  ...
  - Deferred watched-on-skip (v5.3.0): skipping to the next episode from the last quarter (Next button / credits Skip / auto-skip-credits) arms a 60 s grace timer (`state.pending_watch`) that marks the left episode watched — unless the viewer returns to it, which makes a wrong-early credits guess harmless. See [docs/ANALYZER.md § Deferred "watched" when the credits guess is wrong](docs/ANALYZER.md).
...
- [x] **18.3** Server: idle detection. `track_activity` middleware stamps `state.last_activity` on mutating verbs + `/api/search` (routine GET polling excluded). `_machine_in_use()` also treats live VLC playback/pause of non-background content and any active stream/download as in-use, so a reboot never interrupts watching or a download. The reboot loop additionally ORs in `_prep_in_progress()` (HLS-prep / STT actively encoding any queue, or priority prep queued) so an idle reboot never throws away in-flight prep that can't checkpoint (v5.5.1).
...
- [x] **23.12** (v5.5.0) **Post-update log archival + version banner.** After a successful `setup.py` step, `_run_apply` drops a `logs/.rotate_pending` marker (it can't archive/clear `streamlink_app.log`/`hls.log` inline — the live process holds them open, fatal on Windows). On the next process start, `_init_logging()` consumes the marker *before adding any handler* and `_archive_old_logs()` zips every current log into `logs/logs_old_<timestamp>.zip`, then clears the originals (best-effort per file: open files are still captured by read; a file another process holds open is left in place). Every process start now logs a `StreamLink v<x.y.z> starting — branch=… commit=…` banner (`_log_version_banner`, called in `lifespan`), and the stale `UI_VERSION` constant is resynced to the badge. Docs: GOTCHAS, ADMIN, CHANGELOG.
