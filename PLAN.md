...
- [x] **16.14** Fix: HLS playback stalled with a fatal `fragLoadError` even though the manifest + audio/sub dropdowns loaded. The fmp4 init segment was the one bundle filename left un-templated — ffmpeg's default `%v` expansion didn't match the `#EXT-X-MAP:URI=` it wrote into each variant playlist, so the player 404'd the init segment and bailed before any frame decoded. Pinned `-hls_fmp4_init_filename "init_%v.mp4"` (now `init_video.mp4` / `init_audio_0.mp4`, matching the segment scheme); bumped `OFFLINE_CACHE_VERSION` → `v5-hls` so broken bundles rebuild; the hls.js `ERROR` handler now logs the failing URL + HTTP code (alert shows filename + code) for self-diagnosis. Docs: STREAMING.md + GOTCHAS.md.
