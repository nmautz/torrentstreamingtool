...
- [x] **5.4** Subtitle Download: Find subtitles for the track by hash or name
  - Aggressive local sidecar discovery (v4.18.0): VLC only auto-loads the exact-stem sub in the video's own folder, so subs in `Subs/`/`Subtitles/` folders (flat single-movie, or per-episode subfolders in season packs) were missed. `_discover_local_subs` now sweeps the video's dir + sibling subtitle folders (and one level up), loads **all** matches into VLC via `addsubtitle`, and selects the `default_language` track (embedded ahead of sidecars). Languages are read from filenames incl. full names (`2_English.srt`, `3_Brazilian.Portuguese.srt`) via `_parse_sub_lang`. AI sidecars fold into the same discovery. See [docs/GOTCHAS.md](docs/GOTCHAS.md).
  - On-device parity (v4.19.0): `_list_sidecar_subs` (the on-device/in-browser sub list) now uses the same `_discover_local_subs` sweep, so `Subs/`-folder subs appear there too, independent of HLS prep. The `/api/library/{id}/subtitle` endpoint serves subfolder subs (path validated inside the item's media tree) and converts `.ass`/`.ssa` → WebVTT via ffmpeg on demand; the player flashes a "Loading subtitles…" hint until the `<track>` lands. See [docs/STREAMING.md](docs/STREAMING.md).
...
- [x] **6.7** Seeding & Bandwidth limits from admin: **System** tab "Seeding & Bandwidth" card sets qBittorrent's **global** share-ratio limit (stop seeding when reached, files kept — `max_ratio_act=0`) and max up/down speeds (MiB/s, 0 = unlimited). Read/written live via the qBit Web API (`app/preferences`, `transfer/*Limit`) and persisted by qBit; `GET`/`POST /api/admin/qbit-limits`. See [docs/ADMIN.md § Seeding & Bandwidth](docs/ADMIN.md).
...
- [x] **17.3a** Prep gating: the handoff-to-device button greys out (with a "Not prepped for on-device streaming" note) when the current file has no `.offline_cache` MP4 / isn't Safari-native. Readiness from `prepFileState` + a per-file `/prep-status` check (`_handoffReadyState` / `_maybeRefreshHandoffReady`); flips to active once prepped. Tapping while not prepped shows a toast. (Footer button only — the fullscreen tile is now hold-to-prep, see 17.3b.)
- [x] **17.3b** Fullscreen To-Device tile hold-to-prep: when the current file isn't stream-ready the `#fcHandoffBtn` tile shows **"Prep for Device?"** instead of greying; holding starts an interactive `/offline-prepare {bulk:false}` encode **while VLC keeps playing** (TV not stopped), the tile shows live **"Prepping 42%"** with a `#fcHandoffBar` fill, then flips to **"Play To Device"** when ready. `_renderFcHandoff` (state machine) + `fcDeviceTileHold` (hold dispatcher) + `prepCurrentForDevice` + `_finishFcPrep`; state in `app._fcPrepFile` / `app._fcPrepPct`. macOS-no-HLS keeps the legacy greyed note. **Interactive prep takes precedence over bulk** (`_preempt_running_bulk` boots the in-flight bulk encode; `_interactive_hls_pending` makes bulk jobs defer/yield the single slot; `_requeue_offline_job` + the `_preempted` branch re-queue the booted bulk file) so holding the tile starts *now* instead of queueing behind overnight/idle/manual prep.
...
- [x] **20.9** Auto-prep on play (v4.16.0): every VLC library play HLS-preps the playing episode for on-device then the rest of the playlist one at a time (`_maybe_start_play_prep` → `_play_prep_chain` → `_start_interactive_prep_job`, tracked on `state.play_prep_task`; new play cancels the prior chain). The current episode is skipped when resumed with <`PLAY_PREP_TAIL_SECS` (300 s) left → start at the next. Jobs are **interactive** so they run regardless of overnight/idle settings and live activity (the bulk pause gate + `_activity_kick` don't touch them) and preempt in-flight bulk. Config `library.json → settings.play_prep` (`enabled`, default **on**); `GET/POST /api/admin/play-prep`; admin **System → Auto-Prep on Play** toggle. Off on macOS (no HLS). Docs: STREAMING.md, ADMIN.md, API.md, LIBRARY_DATA.md, BACKEND.md, CHANGELOG.
- [x] **20.10** Admin force-prep (v4.24.0): an admin-initiated prep that viewers and host activity **cannot** stop. New `"admin"` prep queue — like `interactive` it ignores the bulk pause gate + `_activity_kick` and preempts bulk (`_priority_hls_pending` now counts both non-bulk queues), but the admin can halt it via `_stop_admin_prep(hard)` gated on `state.admin_prep_stop`: **soft** lets the in-flight file finish (cached) then cancels the queued rest; **hard** also `terminate()`s the running ffmpeg (`_admin_stopped`, partial dropped). Not auto-resumed; a fresh start clears the gate. Helpers `_enqueue_admin_prep` / `_start_admin_prep_job` / `_force_prep_status`. `GET/POST /api/admin/force-prep {item_id?}` + `POST /api/admin/force-prep/stop {hard}`; admin **System → Force Stream Prep** card (whole-library or per-item scope dropdown, live status poll). Off on macOS (no HLS). Docs: STREAMING.md, ADMIN.md, API.md, CHANGELOG.
...
- [x] **28.6** (v4.7.1) Fix: a finished **partial** selection looked fully downloaded. `library_download_monitor` now flips `ready` only when **`_all_nonskip_complete`** (every non-skip file done — not qBit's torrent state, which calls a torrent with priority-0 skipped/idle files "complete"), so ready + audio fingerprinting wait for the kept set and idle-deferred files aren't abandoned. `_analyzable_files` excludes skip files from analysis + `_item_skip_status`; `get_item_files` reports true qBit per-file `complete`/`dl_pct` for ready items too; card shows **⊘ Partial** (`download_partial`) and the episode picker marks **⊘ Not downloaded** (dimmed, not playable). Docs: BACKEND, API, GOTCHAS, CHANGELOG.
- [x] **28.7** (v4.8.0) Full per-file control in the **ready** state, incl. "download this skipped file now". `_apply_item_schedule` reconciles + reactivates a finished item (`ready`→`downloading`) when a non-skip file is no longer on disk; the `download-schedule`/`file-schedule` endpoints route through it. `get_item_files` derives `complete`/`dl_pct` from live qBit progress regardless of mode (a downloaded-then-skipped file still reads complete). Frontend: the Files expander shows on ready partial cards (and re-expands across re-renders); the episode picker gains a **⬇ Download** button (`epDownloadSkipped`) on skipped rows + a live "Downloading…%" state. Docs: API, BACKEND, FRONTEND, CHANGELOG.
- [x] **28.8** (v4.9.0) Episode-picker download scheduling at every level + fix the per-episode download button. Picker control bar (`epSchedSeason`/`epSchedItem`): **This season / All files** Now·Idle·Skip and (multi-season) whole-torrent **All** Now·Idle (`download-schedule {reset_files}` clears overrides so even skipped files are included). Per-episode device-download `<a>` now shows **only for `complete` files**; not-on-disk files get a host **⬇ Download** (skipped/idle) or a live % (downloading). `get_item_files` hardened with a basename fallback for `complete`/`dl_pct` and returns `has_torrent`. Docs: API, FRONTEND, CHANGELOG.
...
- [x] **30.3** (v4.13.0) Intensity presets (Light / Medium / Max). `NIGHT_MODE_ARGS` → `NIGHT_MODE_PRESETS` (mirrored in run.py/watchdog.py); `settings.vlc_night_mode_preset` + `state.vlc_night_mode_preset` remembered independently of on/off. `POST /api/settings/night-mode` takes optional `{night_mode?, preset?}` (merged), relaunches VLC only when the running filter changes. Settings-menu-only picker (`#psNightModePreset` / `setNightModePreset`); not in the fullscreen UI. Docs: API, BACKEND, FRONTEND, LIBRARY_DATA, GOTCHAS, CHANGELOG.


## Milestone 31 — Unified subtitle policy + explicit VLC on/off

Three subtitle problems: online search buried English (defaulted to all languages), nothing controlled which language/track auto-selected, and VLC turned subs **on** even when the UI said off. Fix with one admin-configured policy (preferred language + on/off-default + auto-search) that every viewer inherits and can override, with VLC told explicitly what to do.

- [x] **31.1** (v4.14.0) Data + config. `settings.subtitles` (`default_language` eng-default·"" = Any, `on_by_default` false, `auto_search` true) via `_subs_cfg`; `_stt_cfg.default_language` re-sourced from it (one unified language); per-profile `profile.subtitles_on` override. Migration: absent block seeds from legacy `stt.default_language` else "eng".
- [x] **31.2** (v4.14.0) Backend selection. `_apply_subtitle_policy` (from `_apply_track_prefs`) sends VLC an **explicit** `subtitle_track` every play/prev/next — off `-1`, else preferred-lang track → online auto-search (`_auto_fetch_subtitle`) → AI sidecar → off. `_vlc_subtitle_tracks` factored from `/api/vlc/tracks`; `_download_and_attach_subtitle` refactored from the download endpoint (auto-search uses `save_pref=False`). `search_subtitles` blank `lang` → preferred (`all` = every language). `state.subtitle_default_language` + `state_snapshot`. Profile payloads carry `subtitles_on`.
- [x] **31.3** (v4.14.0) Endpoints. `GET`/`POST /api/admin/subtitles`; `POST /api/profiles/{id}/subtitles`; `POST /api/admin/stt` drops `default_language`.
...
## Milestone 34 — On-demand (just-in-time) streaming

Goal: playing a **non-prepped** file on-device starts almost immediately instead of waiting for the whole file to encode. A virtual HLS playlist is served from the source duration alone; segments are transcoded just-in-time as the player fetches them (Jellyfin/Plex model). On-demand is a *supplement* — an existing full `.offline_cache/<sha>/` bundle still takes precedence, and the server kicks off the full bundle prep in the background so the next play is bundle-mode. Single source-resolution rendition + one switchable audio track; ABR/seamless-multi-audio stay bundle-only.

- [x] **34.1** (v5.0.0) Backend session manager. `_od_sessions` keyed by `_od_session_key(src, audio_idx)`; `_od_build_ffmpeg_args` (mpegts segments, `-ss` input-seek + `-force_key_frames expr:gte(t,n_forced*6)` + `-start_number` so seg N == `[N*6,(N+1)*6)`; always transcodes video, NVENC transparent tier when present); `_od_start_encode` / `_od_teardown` / `_od_reaper` (registered in `lifespan`, 90 s idle reap + `OD_MAX_SESSIONS` cap). Reuses `_offline_cache_key`, `_ffprobe_full`, `_has_nvenc`, `_ffmpeg_nice_prefix`, `_FFMPEG_SUBPROCESS_KW`. Constants `ONDEMAND_CACHE`, `OD_SEGMENT_SECS`, `OD_SESSION_IDLE_SECS`, `OD_SEG_WAIT_TIMEOUT`, `OD_LOOKAHEAD_SEGS`.
- [x] **34.2** (v5.0.0) Backend endpoints. `POST /api/library/{id}/stream-ondemand` (ffprobe → register session + kick background full prep via `_maybe_start_prep_job` bulk; returns `master_url`/`audios`/`subs`/`saved_tracks`/`default_audio_idx`); `GET /api/library/ondemand/{key}/{filename}` (virtual master/media playlist + JIT segment serve that holds the request open until the `.ts` lands, restarting the encode on a seek; 504 on timeout); `POST /api/library/ondemand/{key}/close` (sendBeacon teardown). 503 on macOS.
- [x] **34.3** (v5.0.0) Frontend. `_lpLoadIndex` now branches: bundle path when prepped, else `_odKeyFromUrl` + `lp.mode="ondemand"` from `stream-ondemand` (no more full-encode wait). `waiting`→"Loading…" / `playing`→hide for the seek/start buffering UX; `lpSetAudio` re-enters `_lpLoadIndex` in OD mode (saved `audio_idx` drives the new session); clip row hidden in OD mode; `fragLoadingTimeOut` raised to 45 s; close-beacon on stop/pagehide. Version badge → 5.0.0.
- [x] **34.4** (v5.0.0) Docs — STREAMING (On-Demand section), API (3 endpoints), GOTCHAS (TS-vs-fmp4 / forced-keyframe alignment / blocking segment serve / concurrent JIT+full-prep), BACKEND (session helpers), CHANGELOG. `.ondemand_cache/` gitignored.


## Milestone 35 — Admin Offline Cache snapshot (instant tab open)

Goal: the admin Offline Cache tab opens instantly instead of re-running the heavy recursive `.offline_cache/` size walk on every visit. The last result is cached and shown with an "As of …" stamp; Refresh forces a fresh on-disk re-check.

- [x] **35.1** (v5.1.1) Backend. `_build_offline_cache_inventory(*, force=False)` keeps a module-level `_offline_cache_inv_snapshot` (guarded by `_offline_cache_inv_lock` so concurrent builds share one walk) and stamps each result with `generated_at`. Reads serve the snapshot; `force=True` re-walks. `_invalidate_offline_cache_inventory()` drops the snapshot. `GET /api/admin/offline-cache?refresh=1` → `force`; all three delete endpoints + the auto-purge eviction invalidate; `cache_autopurge_loop` builds with `force=True` (refreshing the snapshot for free).
- [x] **35.2** (v5.1.1) Frontend. `loadOfflineCache(forceRefresh)` hits `?refresh=1` only for the Refresh button (disabled + *Checking…* while it runs); tab open + post-delete reloads serve the cached snapshot. New "As of …" line (`#offGeneratedAt`, `_fmtAgo`). Version badge → 5.1.1.
- [x] **35.3** (v5.1.1) Docs — ADMIN (snapshot + freshness), API (`generated_at` + `?refresh=1`), CHANGELOG.


## Milestone 36 — Consolidate auto stream-prep into one mode control

Goal: the separate **Overnight Stream Prep** (fixed nightly window) + **Idle Auto-Prep** cards were confusing about what would actually happen. Replace both with a single **Automatic Stream Prep** control with three clear, mutually-exclusive modes; drop the fixed time-window concept entirely (modes are purely activity-based).

- [x] **36.1** (v5.2.0) Config + loop. New `settings.auto_prep` (`mode ∈ always|idle|off`, `idle_minutes`, `on_activity ∈ soft|hard`) via `_auto_prep_cfg` (lazy-migrates from the legacy `idle_prep`/`overnight_prep` keys). `auto_prep_loop` rewritten around `mode`: **always** = prep regardless of activity (re-enqueues new content ~every 5 min while engaged); **idle** = prep only when `_machine_in_use(…, for_prep=True)` is False, falling-edge `_pause_prep(kill = on_activity=="hard")`; **off** = never. `_overnight_prep_cfg`/`_idle_prep_cfg`/`_in_overnight_window` + `OvernightPrepReq`/`IdlePrepReq` removed; `state.overnight_open` → `state.idle_prep_hard`. `_activity_kick` now gated on `idle_prep_hard`. `_download_idle_open`/`_download_idle_configured` re-derived from the mode.
- [x] **36.2** (v5.2.0) Endpoints + UI. `GET/POST /api/admin/auto-prep` (`AutoPrepReq`) replace the four overnight/idle endpoints. Admin **System** tab: one **Automatic Stream Prep** card (mode chooser + idle-only sub-fields) replaces the two old cards (`loadAutoPrep`/`saveAutoPrep`/`setAutoPrepMode`). Download-modal copy updated (`static/index.html`). Version badge → 5.2.0.
...
## Milestone 37 — AirPlay screen-mirror receiver (iPhone → host TV, Windows)

Goal: let a user screen-mirror **arbitrary iPhone content** (TikTok, any app) onto the TV wired to the **Windows host**, via iOS Control Center → Screen Mirroring. iOS only lists a target that advertises AirPlay over Bonjour, and Windows isn't a receiver out of the box — so the host must run AirPlay-receiver software. **Pragmatic/turnkey** approach: install the prebuilt leapbtw/uxplay-windows receiver (bundles GStreamer + Apple Bonjour); StreamLink's value-add is mediating the screen (VLC yields when a mirror connects) + a status hint. No in-app start/stop, no fully-managed receiver, no compiling UxPlay. Windows-only, opt-in.

- [x] **37.1** (v5.3.0) `setup.py` — `install_airplay_receiver()` (Windows, opt-in, default No): detect via `uxplay_win_candidates()` / install via `_resolve_airplay_win_installer_url()` + `_download_with_progress` + GUI installer; `_bonjour_service_present()` check; records `_UXPLAY_WIN`. Wired into `detect_tools`, `write_env`/`merge_tool_paths` mappings, and `main()`.
- [x] **37.2** (v5.3.0) `run.py` — `start_airplay()` (gated on `AIRPLAY_RECEIVER=1`): `net start "Bonjour Service"` + `launch_bg` the receiver; `find_airplay()`. Best-effort, never blocks startup.
- [x] **37.3** (v5.3.0) `main.py` — `state.airplay_available`/`airplay_active` (+ in `state_snapshot`), `airplay_mirror_watch` lifespan task (`_find_airplay_hwnds_windows` / `_focus_airplay_windows`) minimizes VLC + reclaims it on falling edge; `airplay_active` guards added to `vlc_focus_and_fullscreen` + `background_video_loop` (mirrors the `youtube_active` pattern). Env-overridable detection hints.
- [x] **37.4** (v5.3.0) Frontend — `#airplayHint` (Search tab) + `_renderAirplayHint`: shown when `airplay_available`, flips to "Mirroring now" when `airplay_active`. No start/stop button. Version badge → 5.3.0.
- [x] **37.5** (v5.3.0) Decisions — watchdog supervision **skipped** (self-managing tray app, no health port); receiver downloaded from upstream at setup, not redistributed (GPL).
- [x] **37.6** (v5.3.0) Docs — new AIRPLAY.md; ARCHITECTURE (topology), RUNTIME (`start_airplay`), SETUP (install + `.env`), API (state fields), GOTCHAS (Bonjour/subnet/marker footguns), CLAUDE (doc index), CHANGELOG.
- [x] **37.8** (v5.3.1) Installable from the admin panel + autoupdater-safe. Root cause of "not available": the autoupdater skips every `setup.py install_*` step, so `install_airplay_receiver` never ran. Added `airplay` to the **Optional Components** pipeline — `_COMPONENT_KEYS`, the `airplay` branch of `_run_component_install` (download uxplay-windows installer → `tools/airplay/`, pre-enable `AIRPLAY_RECEIVER=1`, best-effort launch wizard, guidance note), `_component_status_payload` entry, the `airplay` Windows-only guard on `/api/admin/components/install`, and the admin.html component row (order array + "Get installer" label + note rendering).
- [x] **37.9** (v5.3.1) Availability is now **detection-based** (`_airplay_installed` — receiver binary on disk, not the env toggle) so an admin install goes live without a restart; `airplay_mirror_watch` loops continuously and self-refreshes `state.airplay_available` every ~15 s (was an early-return at startup). Docs (AIRPLAY/ADMIN/SETUP/API), CHANGELOG, version badge → 5.3.1.
- [x] **37.10** (v5.4.0) Fixed the "Couldn't resolve installer" failure: the releases are `.msi` + `.zip` (x64 + arm64-UNTESTED), no `.exe`. `_resolve_airplay_win_installer_url` now prefers the x64 `.msi`, falls back to `.zip`, excludes arm64/untested; the install branch + setup.py run `.msi` via `msiexec` and extract `.zip`.
- [x] **37.11** (v5.4.0) **Self-elevating installs.** New `WINDOWS_ADMIN_USER`/`WINDOWS_ADMIN_PASSWORD` (Settings + `ENV_KEY_FEATURES`, so the existing admin env-keys editor is the UI; `_reload_settings` makes them live). The AirPlay `.msi` installs silently (`msiexec /qn`) when creds are set, else GUI fallback. Security caveats documented (plaintext `.env`, local-admin requirement).
- [x] **37.12** (v5.4.1) Switched the runtime elevation attempt to `CreateProcessWithLogonW` after schtasks `/Create` was denied to the non-admin server. (Superseded by 37.13 — `CreateProcessWithLogonW` yields a UAC-filtered token → MSI 1603.)
- [x] **37.13** (v5.4.2) **Install-time elevation helper (the working design).** A standard-user process can't silently elevate (schtasks-create denied; seclogon token filtered → 1603). So: `_run_elevated_windows` now runs directly if already elevated, else **triggers a pre-registered `/RL HIGHEST` task** (`StreamLinkElevate`) via `.elevate\run.cmd` + `.elevate\result.txt`. `daemon.py _windows_register_elevation_helper` creates that task during the elevated `--install` when `WINDOWS_ADMIN_USER`/`PASSWORD` are set (creating a HIGHEST task needs elevation; *running* it doesn't), and removes it on uninstall. AirPlay `.msi` install adds `/l*v` logging + remedy-on-failure. `.elevate/` gitignored. **Requires re-running `python run.py --install` once** after setting creds. Docs (AIRPLAY/GOTCHAS/DAEMON_WATCHDOG), CHANGELOG, version → 5.4.2.
- [ ] **37.7** Verify on a real Windows host + iPhone (cannot test from macOS dev box): install from Admin → Optional Components, finish the wizard + Refresh → Installed; dashboard hint appears without restart; receiver appears in Screen Mirroring; mirror lands on the TV display; VLC yields/restores; confirm the `_find_airplay_hwnds_windows` markers (set `AIRPLAY_*_HINTS` if needed); confirm `_UXPLAY_WIN`/`AIRPLAY_RECEIVER` survive an autoupdate.
