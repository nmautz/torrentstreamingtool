...
- [x] **6.5** Off-LAN access gate: force HTTPS for non-LAN clients, require `SITE_PASSWORD` (`.env`) before any `/api/*` responds, mint a 60-min session cookie on login with per-IP brute-force lockout. Admin panel is now LAN-only (404 from off-LAN). Admin **Remote Access** tab toggles the gate on/off and shows a requirements checklist (password set, cert present, HTTPS process listening) so misconfiguration is surfaced. See [docs/SITE_AUTH.md](docs/SITE_AUTH.md).
