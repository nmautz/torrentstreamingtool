...
- [x] **37.11** (v5.4.0) **Self-elevating installs.** New `WINDOWS_ADMIN_USER`/`WINDOWS_ADMIN_PASSWORD` (Settings + `ENV_KEY_FEATURES`, so the existing admin env-keys editor is the UI; `_reload_settings` makes them live). `_run_elevated_windows(cmdline)` runs elevated with no UAC via a one-shot `schtasks /RU /RP /RL HIGHEST` task (poll Last Result, delete). The AirPlay `.msi` installs silently (`msiexec /qn`) when creds are set, else GUI fallback. Security caveats documented (plaintext `.env`, schtasks cmdline exposure, local-admin requirement). Docs (AIRPLAY/ADMIN/GOTCHAS), CHANGELOG, version badge → 5.4.0.
