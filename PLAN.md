...
- [x] **32.5** (v5.0.1) Fix: live resume never started — hls.js fetched seg 0 before the resume seek, thrashing the encoder (restart 0 ⇄ resume-seg). Pin hls.js `startPosition` to `lp.resumeSec` in live mode so it requests the resume segment directly. GOTCHAS note added.
