...
- [ ] **37.7** Verify on a real Windows host + iPhone (cannot test from macOS dev box): receiver appears in Screen Mirroring, mirror lands on the TV display, VLC yields/restores, confirm the `_find_airplay_hwnds_windows` markers (set `AIRPLAY_*_HINTS` if needed).
